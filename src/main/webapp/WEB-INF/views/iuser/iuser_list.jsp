	...
	<table class="ui yellow celled table">
		...
		<tbody>
			<c:forEach var="row" items="${dto}">
				<tr>
					<td>
						<a href='${path}/iuser/detail/${row.u_no}'>${row.u_no}</a>
					</td>
					...
					<td>${row.u_admin}</td>
					<td>
						<a href='${path}/iuser/modify/${row.u_no}'> <i class="edit icon"></i></a> <a href='${path}/iuser/delete/${row.u_no}' onclick="return confirm('本当に削除しますか。');"> <i class="trash alternate icon" style="color: red;"></i>
						</a>
					</td>
				</tr>
			</c:forEach>
